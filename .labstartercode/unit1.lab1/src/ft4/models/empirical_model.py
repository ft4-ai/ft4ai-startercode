...
class EmpiricalModel(L.LightningModule):
    ...
    def __init__(
        ...
        self.n = int(n)
        self.w = self.n - 1
        """w (context window): The number of preceding tokens we condition on to predict the next one"""

        self.vocab_size = vocab_size
        assert (self.vocab_size ** self.w) <= (2**63 - 1), f'{n=} is too big for {vocab_size=} (must encode to < 64 bits)'
        ...
        Observed ngrams are split up into prefix (w tokens) and suffix (the last token).
    ...
    def _freq2logits(self, freq: Tensor) -> Tensor:
        ...
        freq:   Count of each token in the vocab
                Shape: (*, V)
        logits: Logit of selecting that token
                Shape: (*, V)
        where:
         * = Any number of dimensions of any shape
         V = vocab size

        """
        freq = freq.clamp_min(1e-12) # Avoid zeros for unseen targets
    ...
    def _suffix_freq(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        returns: Empirical suffix frequency: 
                 For each pos in the seq, a tally over the vocab for the *next* token 
                 showing how often that token followed the w.
                 Shape: (B,L,V) float.
        where:
         B = batch size
         L = tokens per sequence
        ...
        # If w == 0, there's no context; we simply return the unigram token frequency
        if self.w == 0:
        ...
        # Otherwise, w >= 1, so we use a sliding window to collect the most w recent tokens at every pos
        # To give us a full prefix for the first pos, we pad the seq with w-1 tokens on the left
        pad_len = self.w - 1
        ...
        for b in range(B):
            ...
            # Initialize prefix with the first (w-1) items (no-op when w==1)
            prefix = 0
            ...
            for i in range(L):
                prefix = prefix * self.vocab_size + int(seq[i + self.w - 1])  # push right
    ...
    def training_step(self, batch: dict[str, Tensor], batch_idx: int):
        ...
        # Pad each sequence with w PADs on the left and 0 padding on the right
        # This allows us to to have a complete ngram at each pos, including the first
        padded = F.pad(tokens, (self.w, 0), value=self._padding_token) # shape: (B, L+w)
